...
async def test_create_message(db, default_user):
    message = await Messages.create(db, user_id=default_user.id, chat_id=None, body="__test_create_message")
    assert isinstance(message, models.Message)


@pytest.mark.asyncio
async def test_get_all_messages(db, default_user):
    msg_1 = await Messages.create(db, user_id=default_user.id, chat_id=None, body="__test_create_message_1")
    msg_2 = await Messages.create(db, user_id=default_user.id, chat_id=None, body="__test_create_message_2")
    msg_3 = await Messages.create(db, user_id=default_user.id, chat_id=None, body="__test_create_message_3")
