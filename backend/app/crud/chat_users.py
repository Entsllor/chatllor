...
class ChatUserCrud(BaseCrudDB):
    ...
    async def get_user_chats(self, user_id: int, options: GetManyOptions = None) -> list[models.ChatUser]:
    ...
    async def get_chat_users(self, chat_id: int, options: GetManyOptions = None) -> list[models.ChatUser]:
