...
@pytest.mark.asyncio
async def test_create_chat(client, urls, auth_header):
...
@pytest.mark.asyncio
async def test_failed_create_chat_unauthorized(client, urls, auth_header):
    response = await client.post(url=urls.create_chat, json=CHAT_CREATE_DATA.dict())
    assert response.status_code == status.HTTP_401_UNAUTHORIZED


@pytest.mark.asyncio
async def test_delete_chat(client, urls, auth_header, chat_with_default_user):
...
@pytest.mark.asyncio
async def test_failed_delete_chat_user_not_in_the_chat(client, urls, auth_header, empty_chat):
...
@pytest.mark.asyncio
async def test_failed_delete_unauthorized(client, urls, auth_header, empty_chat):
...
@pytest.mark.asyncio
async def test_read_chats(client, urls, empty_chat):
